...
component.</li><li>TIP Voting: There's a master workflow case for the TIP itself.
...
'active', 'completed', 'canceled', or 'suspended'.</li><li>When the action gets enabled, a callback will create child
...
remaining 'active' child cases will be marked 'canceled'.</li><li>If the action should ever get un-enabled, a callback will
...
meaning that there are active child cases. XXXXXXXXXXXXXX do we
need this?</li><li>
<b>Completed</b>. The action has completed executing. The row
will still stay around so we have a history of what was executed
when and we're able to count the number of times a given action was
executed.</li><li>
<b>Canceled</b>. The action was enabled, but the case's state
changed before the action was triggered. (Note: This is not
necessary, we could just delete the row instead.)</li><li>
<b>Refused</b>. The action had its database-driven
...
the state</li><li>If there's no row in enabled actions for "ask client", execute
"ac-init", which will set case_enabled_actions.state_id to
ac-asking for case_id #1 and action_id "ask client".</li><li>Look in case_enabled_actions.state_id for case_id #1 and
...
</li><li>Assigned? It's assigned if: trigger_type = 'user', state-action
map has assigned_p = true, it has an assigned_role.</li><li>What's the assigned role? (For dynamic actions, there may not
be an assigend role, if the assignees are in the
...
lists or in reminder emails until it's un-suspended.</li><li>In the UI, we want to be able to distinguish between cases that
...
bugs in "open" or "resolved" states are considered active and
should be counted as bugs needing attention, whereas those in
...
current_timestamp + timeout.</li><li>If the action has a timeout of 0, then call
...
longer enabled, because the workflow's state was changed by some
other action.</p><ol><li>If the action has any child cases, these will be marked
...
'per_user', create one case per member/user of that role. If more
roles have per_member/per_user setting, then the cartesian product
of child cases are created (DESIGN QUESTION: Would this ever be
relevant?)</li><li>If there is any ActionEnabled callback, execute that (only the
...
particular for an in-progress state.</li><li style="color: gray;">(Not needed) <b>Action.OnUnEnable</b>:
Gets called when an action that used to be enabled is no longer
enabled. Is not called when the action fires.</li><li style="color: gray;">(Not needed)
<b>Action.OnChildCaseStateChange -&gt; (output, CompleteP)</b>:
Called when a child changes its case state
