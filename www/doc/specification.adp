...
<p>I've finally come to the realization that we'll be better off in
...
can leave the state unaltered.</p>
...
the bug-tracker example above.</p>
<h2>Workflows</h2>
...
<p>There's also a short_name, so you can easily distinguish between
...
components in the bug-tracker.</p>
<p>A workflow is also associated with an <b>object type</b>. The
...
assignment through a nice web-based user interface.</p>
...
workflow, to the assignments, etc.</p>
<p>A workflow can have <b>side-effects</b>, which fire when
...
to format the title of the log entry for display.</p>
<h2>Roles</h2>
...
<p>In the definition of a workflow, you can select <b>an ordered
...
and if non is specified, use the project maintainer".</p>
...
<p>Default assignment is done in a lazy fashion, in that we don't
...
tasks.</p>
<h3>Reassignment</h3>
<p>Now, let's look at what happens when you want to reassign a role
to someone else. the</p>
<ol>
...
<h2>Actions</h2>
<p>In order to determine who are <i>supposed</i> to perform an
...
<p>Actions can also have <b>side-effects</b>, which simply means
...
and to the workflow tables, have been completed.</p>
<h2>States</h2>
...
action, the workflow can be pushed into a new state.</p>
...
<p>States have almost no information associated with them, they're
simply used to govern which actions are available.</p>
<h2>Cases</h2>
...
<p>The <b>GetObjectType</b> method is used for the service contract
...
is the mother of all objects.</p>
<p>The <b>GetPrettyName</b> method will be run through a
localization filter, meaning that any occurance of the
<code>#<i>message-key</i>#</code> notation will be replaced with a
message catalog lookup for the current domain.</p>
<p>The <b>AssigneeQuery</b> service contract probably needs a
...
you get the second, you won't get the third or fourth, etc.</p>
<p>A special case is that the first notification isn't optional.
...
method and the frequency, though.</p>
...
action.</li></ul>
<p>Nice-to-haves that aren't <i>entirely</i> pie-in-the-sky
include:</p>
<ul>
...
<p>I've looked into pluggable models before, and it's not too
